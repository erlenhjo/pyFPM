...
def positional_calibration_recovery_algorithm(
    ...
    recovered_object_guess = initialize_high_res_image(low_res_images, update_order, scaling_factor, object_phase_correction=1)
    

    recovered_object_fourier_transform, recovered_pupil, calibration_values = \
        simulated_anealing(recovered_object_guess, update_order, low_res_images, low_res_CTF, LED_indices,
...
def simulated_anealing(recovered_object_guess, update_order, low_res_images, low_res_CTF, LED_indices,
    ...
    recovered_object_fourier_transform_guess = fftshift(fft2(ifftshift(recovered_object_guess)))
    ...
    for loop_nr in range(max_iterations):
        if loop_nr < 9:
            active_LEDs = 25
            pupil = low_res_CTF
            recovered_object_fourier_transform = recovered_object_fourier_transform_guess
        else:  
...
def simulated_anealing_single_loop(recovered_object_fourier_transform, pupil, low_res_images, low_res_CTF, update_order,
    ...
    for image_nr in range(active_LEDs):
        ...
        best_recovered_low_res_fourier_transform = pupil * recovered_object_fourier_transform[min_y:max_y+1, min_x:max_x+1]                               
...
def error(recovered_low_res_image, raw_low_res_image):
    return np.sum((np.abs(recovered_low_res_image)**2 - raw_low_res_image**2)**2)
