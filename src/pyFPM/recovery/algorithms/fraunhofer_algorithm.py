...
def main_algorithm_loop(recovered_object_guess, loops, use_epry, use_gradient_descent, update_order, low_res_images,  LED_indices,
    ...
    real_space_error_metric = np.zeros(loops)
    low_res_images = low_res_images * scaling_factor_squared

    for loop_nr in range(loops):
        error_metric_numerator = 0
        error_metric_denominator = 0
        for image_nr in range(len(update_order)):
            ...
            error_metric_numerator += np.linalg.norm(raw_low_res_image - np.abs(recovered_low_res_image))**2
            error_metric_denominator += np.linalg.norm(raw_low_res_image)**2
        ...
        real_space_error_metric[loop_nr] = error_metric_numerator/error_metric_denominator


    return recovered_object_fourier_transform, pupil, convergence_index, real_space_error_metric
