...
def calculate_k_vector_range(k_x, k_y, dk_x, dk_y, 
    ...
    k_center_x = (size_high_res_x - 1)/2 - k_x[LED_index_y, LED_index_x]/dk_x
    k_center_y = (size_high_res_y - 1)/2 - k_y[LED_index_y, LED_index_x]/dk_y
    k_min_x = int(np.round(k_center_x - (size_low_res_x - 1) / 2))
    k_max_x = int(np.round(k_center_x + (size_low_res_x - 1) / 2))
    k_min_y = int(np.round(k_center_y - (size_low_res_y - 1) / 2))
    k_max_y = int(np.round(k_center_y + (size_low_res_y - 1) / 2))
