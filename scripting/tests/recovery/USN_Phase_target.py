...
for Y in range(arraysize):
    for X in range(arraysize):
        ...
        image = image[total_image_size_y//2-low_res_image_size_y//2 -1:\
                      total_image_size_y//2+low_res_image_size_y//2 -1,\
                      total_image_size_x//2-low_res_image_size_x//2 -1:\
                      total_image_size_x//2+low_res_image_size_x//2 -1]
...
#### Object guess ####
quadratic_object_phase_term = np.exp(1j * (k0/2*(1/object_to_lens_distance+1/z_LED)*(object_x_values**2+object_y_values**2)))
...
#### Shift ####
shift_x_mesh = (mesh_x_locations/z_LED*object_to_lens_distance)/Ldx
shift_y_mesh = (mesh_y_locations/z_LED*object_to_lens_distance)/Ldy
...
for loop in range(10):
    for n, LED_number in enumerate(spiral_order):
        if not available[LED_number]:
            continue
        
        kx_low, kx_high, ky_low, ky_high = k_shifts[LED_number]
        scale = upscaling**2
        current_spectrum = recovered_spectrum[ky_low:ky_high,kx_low:kx_high]   
        ...
        updated_image = raw_image * projected_image/np.abs(projected_image)# np.exp(1j*np.angle(projected_image))
        
        
        updated_lens_spectrum=scale*fftshift(fft2(ifftshift(updated_image)))
        # does not work very well
        # spectrum_update = np.conj(recovered_pupil) * (updated_lens_spectrum-current_lens_spectrum)\
        #                   / (np.max(np.abs(recovered_pupil))**2)                 
        # pupil_update = np.conj(current_spectrum) * (updated_lens_spectrum-current_lens_spectrum)\
        #                   / (np.max(np.abs(current_spectrum))**2)

        #print(np.max(np.abs(current_spectrum*lens_pupil)), np.max(np.abs(current_spectrum)))


        # if loop ==9 and (n==0 or n==50 or n==100 or n==150 or n==200):
        #     plt.matshow(np.abs(current_spectrum)/(np.max(np.abs(current_spectrum))**2))
        #     plt.matshow(np.abs(current_spectrum)**2/ np.max(np.abs(current_spectrum*lens_pupil)) / (np.abs(current_spectrum)**2 + 1000))
        #     # plt.matshow(np.abs(current_spectrum)**2/ np.max(np.abs(current_spectrum)) / (np.abs(current_spectrum)**2 + 100))
        #     # plt.matshow(np.abs(current_spectrum)**2/ np.max(np.abs(current_spectrum)) / (np.abs(current_spectrum)**2 + 10))
        #     # plt.matshow(np.abs(current_spectrum)**2/ np.max(np.abs(current_spectrum)) / (np.abs(current_spectrum)**2 + 1))
        #     plt.matshow(np.abs(updated_lens_spectrum-current_lens_spectrum))
        #     plt.show()

        spectrum_update = np.conj(recovered_pupil) * (updated_lens_spectrum-current_lens_spectrum) \
                          * np.abs(recovered_pupil) / np.max(np.abs(recovered_pupil)) / (np.abs(recovered_pupil)**2 + 1.0)
        
        pupil_update = np.conj(current_spectrum) * (updated_lens_spectrum-current_lens_spectrum) \
                       * np.abs(current_spectrum)/ np.max(np.abs(current_spectrum)) / (np.abs(current_spectrum)**2 + 1000)
        
        recovered_spectrum[ky_low:ky_high,kx_low:kx_high] += spectrum_update*lens_pupil
        recovered_pupil += pupil_update*lens_pupil

        # plt.matshow(np.log(np.abs(current_spectrum)))
        # plt.matshow(np.angle(current_spectrum))
        # plt.matshow(np.log(np.abs(recovered_spectrum)))
        # plt.matshow(np.angle(recovered_spectrum))
        # plt.matshow(np.abs(recovered_pupil))
        # plt.matshow(np.angle(recovered_pupil))
        # plt.matshow(np.abs(pupil_update))
        # plt.matshow(np.angle(pupil_update))
        # plt.matshow(np.abs(spectrum_update))
        # plt.matshow(np.angle(spectrum_update))
        # plt.show()
